...
if($libRaw -match "NL_TRUSTBUNDLE_PRINCIPALS_COMPAT_V1"){
  Write-Host ("OK: compat already present: " + $LibPath) -ForegroundColor Yellow
} else {
  $append = New-Object System.Collections.Generic.List[string]
  [void]$append.Add("")
  [void]$append.Add("# === NL_TRUSTBUNDLE_PRINCIPALS_COMPAT_V1 ===")
  [void]$append.Add("# Adds support for trust_bundle.principals[] (new) and trust_bundle.signers[] (legacy)")
  ...
  [void]$append.Add("  # New schema: principals[]")
  ...
  [void]$append.Add("  # Legacy schema: signers[]")
  ...
  [void]$append.Add("    `$opt = `"`"")
  [void]$append.Add("    if(`$ns.Count -gt 0){")
  [void]$append.Add("      `$csv = (`$ns -join `",`")")
  [void]$append.Add("      `$opt = (`" namespaces=\`"`" + `$csv + `"\`"`")")
  [void]$append.Add("    }")
  [void]$append.Add("    # allowed_signers: <principal> <publickey> [options]")
  [void]$append.Add("    [void]`$lines.Add((`$principal + `" `" + `$pubkey + `$opt))")
  [void]$append.Add("  }")
  [void]$append.Add("  `$txt = (@(`$lines) -join `"`n`") + `"`n`"")
  [void]$append.Add("  # Prefer project Write-Utf8NoBomLf if present; else local fallback")
  ...
  $libRaw2 = ($libRaw -replace "`r`n","`n") -replace "`r","`n"
  if(-not $libRaw2.EndsWith("`n")){ $libRaw2 += "`n" }
  $newLib = $libRaw2 + $compatText
  Write-Utf8NoBomLf $LibPath $newLib
  Parse-GatePs1 $LibPath
  Write-Host ("PATCH_OK: lib " + $LibPath) -ForegroundColor Green
}

# Hard-overwrite wrapper scripts to use the compat path (thin + deterministic)
...
$makeText = (@($make) -join "`n") + "`n"
Write-Utf8NoBomLf $MakePath $makeText
...
[void]$show.Add("}")
$showText = (@($show) -join "`n") + "`n"
Write-Utf8NoBomLf $ShowPath $showText
