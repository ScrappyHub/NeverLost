...
function RunPSFile([string]$File, [object[]]$Args, [string]$What){
  $Args = @($Args)  # IMPORTANT: single wrap only (no nested array)
...
$payload = Join-Path $payloadDir "watchtower_sigproof_v1e.txt"
$sig     = ($payload + ".sig")
...
try {
  # 1) allowed_signers regen (MUST receive -RepoRoot)
  RunPSFile $make @('-RepoRoot', $RepoRoot) 'make_allowed_signers'

  # 2) payload
  $payloadText = "neverlost.sigproof.v1e`nprincipal=$Principal`nkey_id=$KeyId`nnamespace=$Namespace`nutc=" + (Get-Date).ToUniversalTime().ToString("o") + "`n"
  ...
  # 5) show identity proof (optional but nice)
...
# 6) sigproof receipt always written; hard-fail after if ok=false
. $lib
NL-WriteReceipt $rcpt @{
  schema="neverlost.receipt.v1"
  time_utc=(Get-Date).ToUniversalTime().ToString("o")
  action="watchtower_sigproof_v1e"
  ok=$ok
...
Write-Host "OK: Watchtower sigproof v1e complete" -ForegroundColor Green
