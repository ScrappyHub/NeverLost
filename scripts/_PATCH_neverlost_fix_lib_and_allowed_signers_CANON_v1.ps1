...
$BackupDir = Join-Path $ScriptsDir ("_backup_fix_lib_allowed_signers_CANON_v1_" + $ts)
...
$marker = "# NEVERLOST_PATCH_CANON_V1_SSHKEYGEN"
if ($libText -notmatch [regex]::Escape($marker)) {
  $A = New-Object System.Collections.Generic.List[string]
  [void]$A.Add("# NEVERLOST_PATCH_CANON_V1_SSHKEYGEN")
  ...
  [void]$A.Add("  try { $cmds = @(Get-Command ssh-keygen.exe -All -ErrorAction SilentlyContinue); foreach($c in $cmds){ if ($c -and $c.CommandType -eq `"`Application`" -and $c.Source) { [void]$cands.Add([string]$c.Source) } } } catch { }")
  [void]$A.Add("  try { $w = & where.exe ssh-keygen.exe 2>$null; foreach($p in @($w)){ if (-not [string]::IsNullOrWhiteSpace($p)) { [void]$cands.Add($p.Trim()) } } } catch { }")
  [void]$A.Add("  $uniq = New-Object System.Collections.Generic.HashSet[string]([StringComparer]::OrdinalIgnoreCase)")
  [void]$A.Add("  foreach($p in $cands){ if ([string]::IsNullOrWhiteSpace($p)) { continue }; $pp=$p.Trim(); if (-not (Test-Path -LiteralPath $pp -PathType Leaf)) { continue }; if (-not $uniq.Add($pp)) { continue }; if (NL-TestSshKeygenY $pp) { return $pp } }")
...
Parse-GateFile $Make
Write-Host ("OK: patch complete. Backup at: " + $BackupDir) -ForegroundColor Green
