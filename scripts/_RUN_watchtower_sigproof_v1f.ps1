...
function RunPSFile([string]$File, [object[]]$Args, [string]$What){
  $Args = @($Args) # single wrap; enumerates
...
$payload = Join-Path $payloadDir "watchtower_sigproof_v1f.txt"
$sig     = ($payload + ".sig")
...
try {
  # 1) allowed_signers regen — DIRECT call (no wrapper) to eliminate arg-shape risk
  & $PSExe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $make -RepoRoot $RepoRoot
  if ($LASTEXITCODE -ne 0) { throw ("CHILD FAIL: make_allowed_signers exit_code=" + $LASTEXITCODE) }

  # 2) payload
  $payloadText = "neverlost.sigproof.v1f`nprincipal=$Principal`nkey_id=$KeyId`nnamespace=$Namespace`nutc=" + (Get-Date).ToUniversalTime().ToString("o") + "`n"
  ...
  # 5) show identity proof (optional)
...
NL-WriteReceipt $rcpt @{
  ...
  action="watchtower_sigproof_v1f"
  ok=$ok
...
Write-Host "OK: Watchtower sigproof v1f complete" -ForegroundColor Green
