...
function Patch-File([string]$Path){
  ...
  $before = $raw
  # Replace ANY $var.signers with (Get-TrustBundleSigners $var)
  $re = '(?im)\$([A-Za-z_][A-Za-z0-9_]*)\.signers\b'
  # In .NET regex replacement: $$ => literal $, $1 => group1, so $$$1 => $ + group1
  $raw = [regex]::Replace($raw, $re, '(Get-TrustBundleSigners $$$1)')
  if($raw -eq $before){ Write-Host ("PATCH_NOTE: no $var.signers refs found in " + $Path) -ForegroundColor DarkYellow }
  Write-Utf8NoBomLf $Path $raw
  Parse-GatePs1 $Path
  # Confirm: no remaining ".signers" property access anywhere
  $after = Get-Content -Raw -LiteralPath $Path -Encoding UTF8
  if($after -match '(?im)\.[ ]*signers\b'){
    $hits = @(@([regex]::Matches($after, '(?im)\$[A-Za-z_][A-Za-z0-9_]*\.signers\b') | ForEach-Object { $_.Value } | Select-Object -Unique))
    Die ("PATCH_INCOMPLETE: remaining .signers in " + $Path + " hits=" + ($hits -join ","))
  }
