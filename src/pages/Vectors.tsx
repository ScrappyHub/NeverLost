...
import { runVectors, readFileText } from "../lib/api"
...
export default function Vectors() {
  ...
  return (
    <div style={{ padding: 24 }}>
      <h1 style={{ fontSize: 32, marginTop: 0 }}>Validation Suite</h1>
      <button onClick={handleRun} disabled={busy}>Run Validation Suite</button>

      <div style={{ border: "1px solid #27272a", borderRadius: 16, padding: 16, marginTop: 16 }}>
        <div>Status: {result?.ok ? "PASS" : "Not Run / Fail"}</div>
        <div>Result: {result?.ok ? "Validation Suite Passed" : ""}</div>
      ...
      >
        <h2 style={{ margin: 0 }}>Validation Coverage</h2>
        <div>Positive Validation</div>
        <div>Wrong Principal Negative</div>
        <div>Namespace Negative</div>
        <div>Malformed Shape Negative</div>
      </div>
