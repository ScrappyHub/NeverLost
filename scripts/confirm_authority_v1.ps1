...
if([string]::IsNullOrWhiteSpace($ConfirmTimeUtc)){
  $ConfirmTimeUtc = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")
}
...
$receipt = [ordered]@{
  ...
  schema = "neverlost.operator.authority.confirmed.v1"
  session_id = [string]$sessionObj.session_id
  time_utc = $ConfirmTimeUtc
  trust_bundle_sha256 = $trustSha
}
AppendLineUtf8NoBomLf $LedgerPath (To-CanonJson-Flat $receipt)

Write-Host "AUTHORITY_CONFIRMED_OK" -ForegroundColor Green
