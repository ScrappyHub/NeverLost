...
function NL-LoadTrustBundleInfoV1([string]$RepoRoot){
  $rb = (Resolve-Path -LiteralPath $RepoRoot).Path
  $tbPath = Join-Path $rb "proofs\trust\trust_bundle.json"
...
function NL-GetTrustBundleEntriesCompat([object]$TrustBundleObj){
  ...
  if(($TrustBundleObj | Get-Member -Name principals -MemberType NoteProperty,Property -ErrorAction SilentlyContinue)){
    $arr = @(@($TrustBundleObj.principals))
    $out = New-Object System.Collections.Generic.List[object]
    foreach($p in $arr){
      ...
      $ns = @(@($p.namespaces)) | Where-Object { $_ -ne $null -and ([string]$_).Trim().Length -gt 0 } | ForEach-Object { ([string]$_).Trim() }
      if(-not $principal){ throw "TRUST_BUNDLE_PRINCIPAL_MISSING" }
      if(-not $pubkey){ throw ("TRUST_BUNDLE_PUBKEY_MISSING principal="+$principal) }
      $ns = @(@($ns)) | Sort-Object
      [void]$out.Add(@{ principal=$principal; key_id=$keyId; pubkey=$pubkey; namespaces=$ns })
    }
    return @(@($out))
  }
  if(($TrustBundleObj | Get-Member -Name signers -MemberType NoteProperty,Property -ErrorAction SilentlyContinue)){
    $arr = @(@($TrustBundleObj.signers))
    $out = New-Object System.Collections.Generic.List[object]
    foreach($s in $arr){
      if($null -eq $s){ continue }
      $principal = [string]$s.principal
      $keyId     = [string]$s.key_id
      $pubkey    = [string]$s.pubkey
      $ns = @(@($s.namespaces)) | Where-Object { $_ -ne $null -and ([string]$_).Trim().Length -gt 0 } | ForEach-Object { ([string]$_).Trim() }
      if(-not $principal){ throw "TRUST_BUNDLE_PRINCIPAL_MISSING" }
      if(-not $pubkey){ throw ("TRUST_BUNDLE_PUBKEY_MISSING principal="+$principal) }
      $ns = @(@($ns)) | Sort-Object
      [void]$out.Add(@{ principal=$principal; key_id=$keyId; pubkey=$pubkey; namespaces=$ns })
  ...
  $keys = (@($TrustBundleObj.PSObject.Properties) | ForEach-Object { $_ .Name }) -join ", "
  throw ("TRUST_BUNDLE_SCHEMA_UNKNOWN: expected principals[] or signers[]; top_keys=" + $keys)
}
# Override: NL-WriteAllowedSignersFromTrust (last definition wins)
function NL-WriteAllowedSignersFromTrust([string]$RepoRoot){
  $rb = (Resolve-Path -LiteralPath $RepoRoot).Path
  $tbInfo = NL-LoadTrustBundleInfoV1 $rb
  $entries = NL-GetTrustBundleEntriesCompat $tbInfo.Obj
  $asPath = Join-Path $rb "proofs\trust\allowed_signers"
  ...
  foreach($e in ($entries | Sort-Object principal)){
    ...
    $opt = ""
    if($ns.Count -gt 0){ $opt = (" namespaces=\"" + ($ns -join ",") + "\"") }
    [void]$lines.Add(($principal + " " + $pubkey + $opt))
  }
  $txt = (@($lines) -join "
") + "
"
  if(Get-Command Write-Utf8NoBomLf -ErrorAction SilentlyContinue){ Write-Utf8NoBomLf $asPath $txt } else {
    $t = ($txt -replace "
","
") -replace "
","
