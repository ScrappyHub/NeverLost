...
function Write-Utf8NoBomLf([string]$Path,[string]$Text){ EnsureDir (Split-Path -Parent $Path); $t = ($Text -replace "`r`n","`n") -replace "`r","`n"; if(-not $t.EndsWith("`n")){ $t += "`n" }; [System.IO.File]::WriteAllBytes($Path,(New-Object System.Text.UTF8Encoding($false)).GetBytes($t)) }
...
# Freeze/verify expected namespace set for principal[0]
...
$expectPath = Join-Path $RepoRoot "proofs\trust\expected_namespaces_principal0.txt"
if(-not (Test-Path -LiteralPath $expectPath -PathType Leaf)){
  $txt = (@($nsActual) -join "`n") + "`n"
  Write-Utf8NoBomLf -Path $expectPath -Text $txt
  Write-Host ("EXPECTED_NAMESPACES_WROTE: " + $expectPath) -ForegroundColor Yellow
}
$nsExpected = @(Get-Content -LiteralPath $expectPath -Encoding UTF8) | ForEach-Object { ([string]$_).Trim() } | Where-Object { $_.Length -gt 0 } | Sort-Object -Unique
$a = @($nsActual)
$b = @($nsExpected)
...
# allowed_signers write + first line prefix
