...
[void]$T.Add("  $root = (Resolve-Path -LiteralPath $RepoRoot).Path")
[void]$T.Add("  $tbPath = Join-Path $root 'proofs\trust\trust_bundle.json'")
...
[void]$T.Add("  $names = @()")
[void]$T.Add("  if($TrustBundleObj.PSObject -and $TrustBundleObj.PSObject.Properties){ $names = @($TrustBundleObj.PSObject.Properties.Name) }")
[void]$T.Add("  $hasPrincipals = $names -contains 'principals'")
[void]$T.Add("  $hasSigners    = $names -contains 'signers'")
[void]$T.Add("  $src = $null")
[void]$T.Add("  if($hasPrincipals){ $src = @(@($TrustBundleObj.principals)) } elseif($hasSigners){ $src = @(@($TrustBundleObj.signers)) } else {")
[void]$T.Add("    throw ('TRUST_BUNDLE_SCHEMA_UNKNOWN: expected principals[] or signers[]; top_keys=' + ($names -join ', '))")
[void]$T.Add("  }")
...
[void]$T.Add("  $root = (Resolve-Path -LiteralPath $RepoRoot).Path")
[void]$T.Add("  $tbInfo = NL-LoadTrustBundleInfoV1 $root")
[void]$T.Add("  $entries = NL-GetTrustBundleEntriesCompat $tbInfo.Obj")
[void]$T.Add("  $asPath = Join-Path $root 'proofs\trust\allowed_signers'")
