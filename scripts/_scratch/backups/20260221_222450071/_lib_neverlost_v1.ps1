...
# StrictMode-safe property reads (NO direct $obj.maybe_missing)
...
function NL-GetTrustBundleEntriesCompat([object]$TrustBundleObj){
  ...
  foreach($p in $src){
    ...
    # namespaces can exist at principal-level and/or key-level; we union them
    $nsPrincipal = NL-ExtractNamespacesCompat $p

    if($hasKeys){
      $keysObj = NL-GetPropObj $p 'keys'
      $keys = @()
      if($null -ne $keysObj){ $keys = @(@($keysObj)) }
      ...
      foreach($k in $keys){
        ...
        # union namespaces deterministically (sorted)
        $nsAll = @()
        $nsAll += @($nsPrincipal)
        $nsAll += @($nsKey)
        ...
        [void]$out.Add(@{ principal=$principal; key_id=$keyId; pubkey=$pubkey; namespaces=$nsAll })
      }
    ...
    [void]$out.Add(@{ principal=$principal; key_id=$keyId2; pubkey=$pubkey2; namespaces=$nsPrincipal })
  }
...
function NL-WriteAllowedSignersFromTrust([string]$RepoRoot){
  ...
  foreach($e in ($entries | Sort-Object principal, pubkey)){
    $principal = [string]$e.principal
