...
function First-NonEmptyString([object[]]$vals){
  foreach($v in @($vals)){
...
function Coerce-Array($v){
  # Canonical scalar-collapse fix: always materialize array
  return @(@($v))
}
...
$BackupDir = Join-Path $ScriptsDir ("_neverlost_backup_make_allowed_signers_v10_" + $ts)
New-Item -ItemType Directory -Force -Path $BackupDir | Out-Null
if (Test-Path -LiteralPath $Make -PathType Leaf) {
  Copy-Item -LiteralPath $Make -Destination (Join-Path $BackupDir "make_allowed_signers_v1.ps1.pre_v10") -Force
}

# ---- write NEW make_allowed_signers_v1.ps1 (schema-flexible, StrictMode-safe) ----
...
[void]$M.Add('')
[void]$M.Add('# ---- locate keys array without touching missing properties (StrictMode-safe) ----')
[void]$M.Add('$keys = $null')
[void]$M.Add('if (Has-Prop $tb "keys") { $keys = Coerce-Array (Get-Prop $tb "keys") }')
[void]$M.Add('if (($null -eq $keys -or $keys.Count -lt 1) -and (Has-Prop $tb "trust")) {')
[void]$M.Add('  $t = Get-Prop $tb "trust"')
[void]$M.Add('  if (Has-Prop $t "keys") { $keys = Coerce-Array (Get-Prop $t "keys") }')
[void]$M.Add('}')
[void]$M.Add('if (($null -eq $keys -or $keys.Count -lt 1) -and (Has-Prop $tb "trust_bundle")) {')
[void]$M.Add('  $t = Get-Prop $tb "trust_bundle"')
[void]$M.Add('  if (Has-Prop $t "keys") { $keys = Coerce-Array (Get-Prop $t "keys") }')
[void]$M.Add('}')
[void]$M.Add('if ($null -eq $keys -or $keys.Count -lt 1) {')
[void]$M.Add('  foreach($alt in @("signers","trusted_keys","allowed_keys")){')
[void]$M.Add('    if (Has-Prop $tb $alt) { $keys = Coerce-Array (Get-Prop $tb $alt); break }')
[void]$M.Add('  }')
[void]$M.Add('}')
[void]$M.Add('if ($null -eq $keys -or $keys.Count -lt 1) {')
[void]$M.Add('  $top = @($tb.PSObject.Properties.Name) -join ","')
[void]$M.Add('  $trustProps = ""')
[void]$M.Add('  if (Has-Prop $tb "trust") { $trustProps = "; trust={" + (@((Get-Prop $tb "trust").PSObject.Properties.Name) -join ",") + "}" }')
[void]$M.Add('  $tbProps = ""')
[void]$M.Add('  if (Has-Prop $tb "trust_bundle") { $tbProps = "; trust_bundle={" + (@((Get-Prop $tb "trust_bundle").PSObject.Properties.Name) -join ",") + "}" }')
[void]$M.Add('  Die ("Could not locate keys array in trust_bundle.json. Top-level props={" + $top + "}" + $trustProps + $tbProps) )')
[void]$M.Add('}')
[void]$M.Add('')
...
[void]$M.Add('  $nsRaw     = $null')
[void]$M.Add('  foreach($n in @("namespaces","namespace","ns")){ if (Has-Prop $k $n) { $nsRaw = Get-Prop $k $n; break } }')
...
# hard verify: NL-WriteAllowedSigners must not exist
...
Write-Host ("OK: patch v10 complete. Backup at: " + $BackupDir) -ForegroundColor Green